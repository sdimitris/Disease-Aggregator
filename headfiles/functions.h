...
int tree_attribute(treeNode* root, char* diseaseID,struct tm date1,struct tm date2,int option);
...
int count_attribute(treeNode* root, char* uknown,int option);

int subtree_height(heap_node* root);
char** array(HashEntry*);
heap_node* fill_heap(char**,HashEntry*,char*,int k,char*,char*,int);
